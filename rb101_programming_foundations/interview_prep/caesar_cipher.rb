...
def caesar_cipher(offset, string)
  ...
  string.chars.each do |char|
    ...
    char_code = offset + char.ord
    if char_code <= 122
      encrypted << char_code.chr
    elsif char_code > 122
      encrypted << (96 + (char_code % 122)).chr
    end
  end
  encrypted.join
end
...
# for each character
#   perform a conversion on each character (SEPARATE METHOD)
# don't convert if it isn't alphanumeric, instead just add the char to the array
#   add this converted char to a new array
# join up the chars into a final string
